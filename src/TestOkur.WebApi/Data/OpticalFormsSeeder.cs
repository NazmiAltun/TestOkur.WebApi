namespace TestOkur.WebApi.Data
{
	...
	internal class OpticalFormsSeeder : ISeeder
	{
		...
		public OpticalFormsSeeder(ApplicationDbContext dbContext)
		{
		...
		{
			if (await _dbContext.FormTypes.AnyAsync())
			{
				return;
			}

			var definitions = await SeedFormDefinitions();
			await SeedFormTypes(definitions);
		}

		private async Task SeedFormTypes(List<OpticalFormDefinition> formDefinitions)
		{
			var lessons = _dbContext.Lessons.Where(l => EF.Property<int>(l, "CreatedBy") == default).ToList();
			...
			_dbContext.FormTypes.AddRange(formTypes);

			foreach (var formType in _dbContext.FormTypes.Local)
			{
				var sections = formType.FormLessonSections.ToList();

				foreach (var section in formType.FormLessonSections)
				{
					_dbContext.Entry(section).Property("ListOrder").CurrentValue =
		...
		private async Task<List<OpticalFormDefinition>> SeedFormDefinitions()
		{
			...
			_dbContext.OpticalFormDefinitions.AddRange(formDefinitions);
			await _dbContext.SaveChangesAsync();
